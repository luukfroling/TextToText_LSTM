...
/* Acopy function to avoid all kind of nasty stuff with pointers and such
*/

  static copy(m){
    let result = new Matrix(m.y, m.x);
    for(let i = 0; i < m.y; i++){
      for(let j = 0; j < m.x; j++){
...
*/
  ...
  /* Mutate(matrix) Take in a matrix and mutate it with s specified rate.
  *  Return the new matrix.
  */

  static mutate(m, rate = 0.1){
    let result = new Matrix(m.y, m.x);
    for(let i = 0; i < m.y; i++){
      for(let j = 0; j < m.x; j++){
        if(Math.random() > rate){
          ...
          result.matrix[i][j] = Math.random() * 2 - 1;
        }
      }
    }
